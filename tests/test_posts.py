...
@pytest.mark.parametrize(
    ...
    [
        ...
        (" tle", "pizza  new content", False),
        ("Tallest skyscrapers", "Ok Ok ok", True),
    ],
)
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/102930190290")
    assert res.status_code == 404
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(f"/posts/102930190290", json=data)
    assert res.status_code == 404
